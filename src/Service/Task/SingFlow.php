...
class SingFlow{
	...
	public static function doSingEnt($config,$uid,$wf_actionid)
	{
		...
		if ($wf_actionid == "sok") {//提交处理
			...
			if($back){//第一步
				//更新单据状态
				$bill_update = InfoRepo::updateBill($config['wf_fid'],$config['wf_type'],0);
				if(!$bill_update){
					return ['msg'=>'流程步骤操作记录失败，数据库错误！！！','code'=>'-1'];
				}
				...
				$wf_process = ProcessRepo::getProcessInfo($wf_backflow,$config['run_id']);
