...
use Lh\Workflow\Service\TaskService;
use Lh\Workflow\Repositories\UserRepo;
...
class Workflow{
    ...
    public static function workDoaction($config,$uid)
    {
        ...
        if($sing_st == 0){
            //新增重复业务判断
            $run_check = ProcessRepo::run_check($config['run_process']);//校验流程状态
            if($run_check==2){
                return ['msg'=>'该业务已办理，请勿重复提交！','code'=>'-1'];
            }
            if ($wf_actionid == "ok" || $wf_actionid == "default") {//提交处理
    ...
    //后台验证是否有审核权限
    public static function workflowCheck($flowinfo,$uid,$role)
    {
        $st = 0;
        $message = ['msg'=>'','code'=>'0'];
        if(isset($flowinfo['bill_check'])){
            return ['msg'=>'已审核！','code'=>'-1'];
        }
        if ($flowinfo != -1) {
            if ($flowinfo['sing_st'] == 0) {
                $user = explode(",", $flowinfo['status']['sponsor_ids']);
                //return $flowinfo['status']['auto_person'];
