...
class FlowRepo{
    ...
    public static function processDel($flow_id, $process_id)
    {
        ...
        //操作日志写死
        activitys('backend')
            ->withProperties(['type' => '工作流管理','table'=>'wf_flow_process','id'=>$process_id])
            ->log('admin 通过'.$platform.'登录后台系统:删除步骤', '工作流管理');
    ...
    public static function processDelAll($flow_id)
    {
        ...
        if ($res) {
            ...
            //操作日志写死
            activitys('backend')
                ->withProperties(['type' => '工作流管理','table'=>'wf_flow_process','flow_id'=>$flow_id])
                ->log('admin 通过'.$platform.'登录后台系统:删除所有步骤', '工作流管理');
    ...
    public static function processAdd($flow_id)
    {
        ...
        if (!$processid) {
            ...
            //操作日志写死
            activitys('backend')
                ->withProperties(['type' => '工作流管理','table'=>'wf_flow_process','id'=>$processid->id,'data'=>$data])
                ->log('admin 通过'.$platform.'登录后台系统:新增步骤', '工作流管理');
    ...
    public static function processLink($flow_id, $process_info)
    {
        ...
        //操作日志写死
        activitys('backend')
            ->withProperties(['type' => '工作流管理','table'=>'wf_flow_process','data'=>$process_info])
            ->log('admin 通过'.$platform.'登录后台系统:更新步骤连接', '工作流管理');
        return ['status' => 1, 'msg' => '添加成功！', 'info' => ''];
    }
	/**
     * 属性保存
    ...
    public static function processAttSave($process_id, $datas)
    {
        ...
        if ($ret!==false) {
            ...
            //操作日志写死
            activitys('backend')
                ->withProperties(['type' => '工作流管理','table'=>'wf_flow_process','id'=>$process_id,'data'=>$data])
                ->log('admin 通过'.$platform.'登录后台系统:步骤属性保存', '工作流管理');
    ...
    public static function processAttView($process_id)
    {
        ...
        $process_tos_arr = explode(',', $one['process_tos']); //修复bug  laravel  不支持tp  in 字符串 写法
//        dump($process_tos_arr);
        $process_to_list =  FlowProcess::select('id','process_name','process_type')->whereIn('id',$process_tos_arr)->get();
	 ...
	 *@param $run_flow_process 工作流ID
	 **/
	public static function end_process($run_process,$check_con,$uid=0)
	{
		return RunProcess::where('id',$run_process)->update(['status'=>2,'remark'=>$check_con,'bl_time'=>time(),'uid'=>$uid]);
	}
	/**
	 *更新流程主信息
	 *
...
//        $database = env('DB_DATABASE');
        $database = config('database.connections.tenant.database');//直接读取District 配置好的database
        $table_schema = empty($table_schema) ? $database : $table_schema;
        $table_name = DB::getConfig('prefix').$table_name;
